...
"""
    ...
    async def generate_prototype(self, prompt: str, output_dir: Optional[str] = None):
        ...
        if self.client is None:
            message = "GEMINI_API_KEY is not set; CAD generation is disabled until credentials are configured."
            print(f"[CadAgent WARN] {message}")
            if self.on_status:
                self.on_status({"status": "unavailable", "error": message})
            return None
...
"""
    ...
    async def iterate_prototype(self, prompt: str, output_dir: Optional[str] = None):
        ...
        if self.client is None:
            message = "GEMINI_API_KEY is not set; CAD iteration is disabled until credentials are configured."
            print(f"[CadAgent WARN] {message}")
            if self.on_status:
                self.on_status({"status": "unavailable", "error": message})
            return None
